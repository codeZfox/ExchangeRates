 ...
 *   method to get the proper oldValue. Another case is when the getter is senseless.
 *
 * Example: there is a [View.padding] extension property, which is used to set the equal padding oldValue
 *   for all paddings (left, top, right, bottom). Technically, the left padding is unrelated to,
 *   for example, the bottom padding, so there is just no right oldValue to return.
 *
...
/**
 * Return the grayscale color with the zero opacity using the single color oldValue.
...
/**
 * Return the color with the given alpha oldValue.
 * Examples:
 *   0xabcdef.withAlpha(0xCF) == 0xCFabcdef
 *   0xFFabcdef.withAlpha(0xCF) == 0xCFabcdef
 *
 * @param alpha the alpha channel oldValue: [0x0..0xFF].
 * @return the color with the given alpha oldValue applied.
 */
 ...
 *        (lower or higher than the given oldValue).
 ...
 *
 * @property value the return oldValue if code execution was finished without an exception, null otherwise.
